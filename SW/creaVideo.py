...
command = FFMPEG + " -i " + FILE + " -filter_complex \"select=gt'(scene," + str(
    SUBTITLETHRESHOLD) + ")',metadata=print:file=" + NAME + 'sequenceORIG.dep\" -vsync vfr -y dep' + NAME + "." + EXTENS
...
command = FFMPEG + " -i " + FILE + " -filter_complex \"select=gt'(scene," + str(
    SCENETHRESHOLD) + ")',metadata=print:file=" + NAME + 'scenesORIG.dep\" -vsync vfr -y dep' + NAME + "." + EXTENS
print("Command: " + command)
os.system(command)
...
with open(os.path.join(workingDir, NAME + 'scenesORIG.dep')) as input:
    with open(os.path.join(folder, NAME + 'scenesORIG.lis'), "w") as output:
        ...
        for line in words:
            ...
            command = FFMPEG + " -ss " + line + " -i " + FILE + " -vframes 1 -vsync vfr -y " + os.path.join(folder3,
                                                                                                            line + "-" + NAME + "-SHOT.jpg")
            print("Command: " + command)
            os.system(command)
...
print("Command: " + command)
os.system(command)
command = PERL + ' ' + os.path.join(swDirectory, "inScene.pl") + ' ' + os.path.join(folder,
                                                                                    NAME + "scenesORIG.lis") + ' < ' + os.path.join(
    workingDir, NAME + ".srt") + ' > ' + os.path.join(folder, NAME + ".srt.dep")
print("Command: " + command)
os.system(command)
command = PERL + ' ' + os.path.join(swDirectory, "subtitulos.pl ") + ' 100000 0 <' + os.path.join(folder,
                                                                                                  NAME + ".srt.dep") + ' >' + os.path.join(
    folder, NAME + "srt.lis")
print("Command: " + command)
os.system(command)
...
if copyResolution == 0:
    command = FFMPEG + " -i " + FILE + " -vcodec libx264 -acodec aac -map 0:a:" + str(
        TRACKAUDIO) + " -map 0:v:0 -vf subtitles=" + NAME + '.srt -y ' + os.path.join(folder, NAME + "ORIG.mp4")
    print("Command: " + command)
    os.system(command)
    FILE2 = os.path.join(folder, NAME + "ORIG.mp4")
else:
    command = FFMPEG + " -i " + FILE + " -vcodec libx264 -acodec aac -map 0:a:" + str(
        TRACKAUDIO) + " -map 0:v:0 -vf subtitles=" + NAME + ".srt,scale=" + str(
        copyResolution) + ':-2 -y ' + os.path.join(folder, NAME + "-ORIG.mp4")
    print("Command: " + command)
    os.system(command)
    FILE2 = os.path.join(folder, NAME + "-ORIG.mp4")
...
command = FFMPEG + " -i " + FILE + " -filter_complex \"select=eq'(pict_type,PICT_TYPE_I)'\" -vsync vfr -y " + NAME + "dep." + EXTENS + " 2> " + os.path.join(
    folder, NAME + "iframes" + EXTENS + ".dep")
print("Command: " + command)
os.system(command)
...
with open(os.path.join(folder, NAME + "iframes" + EXTENS + ".dep")) as input:
    with open(os.path.join(folder, NAME + "iframes" + EXTENS + ".lis"), "w") as output:
        ...
        for line in words:
            ...
            command = FFMPEG + " -ss " + timesec + " -i " + FILE + " -vf \"drawtext=fontsize=10:fontcolor=yellow:box=1:boxcolor=black:x=(W-tw)/4:y=H-th-2: text=' TIME=" + timesec + "'\" -vframes 1 -vsync vfr -y \"" + os.path.join(
                folder0, timesec + "-" + NAME + "-IFRAME.jpg\"")
            print("Command: " + command)
            os.system(command)
...
with open(os.path.join(folder, NAME + "srt.lis")) as input:
    for line in input:
        ...
        command = FFMPEG + " -ss " + line2 + " -i " + FILE2 + " -vf \"drawtext=fontsize=10:fontcolor=yellow:box=1:boxcolor=black:x=(W-tw)/4:y=H-th-2: text=' TIME=" + line2 + "'\" -vframes 1 -vsync vfr -y \"" + os.path.join(
            folder1, line2 + "-" + NAME + "-SRT.jpg\"")
        print("Command: " + command)
        os.system(command)
...
for second in range(1, duration, samplingRate):  # Desde 1 hasta el final, cada samplingRate
    ...
    os.system(FFMPEG + " -ss " + str(
        second) + " -i " + FILE + " -vf \"drawtext=fontsize=10:fontcolor=yellow:box=1:boxcolor=black:x=(W-tw)/4:y=H-th-2: text=' TIME=" + str(
        second) + "'\" -vframes 1 -vsync vfr -y \"" + os.path.join(folder2,
                                                                   str(float(second)) + "-" + NAME + "-TIME.jpg\""))
...
with open(os.path.join(folder, NAME + ".srt"), encoding="utf8") as input:
    ...
    for line in words:
        ...
        if (subtitleInit == 1):  # Si es inicio
            ...
            for second in range(int(secondInit),
                                int(secondFinish) + 1):  # Desde incio hasta el final, añado los segundos a la lista
...
for image_file in imgs0:
    ...
    for seconds in range(REDUNDANCY * (-1), REDUNDANCY + 1):
        ...
        if ((int(
                image_file.split("-")[0].split(".")[0]) + seconds) in usedSecondsList):  # Si es un fotograma redundante
            usedSecond = 1
...
for image_file in imgs2:
    ...
    for seconds in range(REDUNDANCY * (-1), REDUNDANCY + 1):
        ...
        if ((int(image_file.split("-")[0].split(".")[
                     0]) + seconds) in usedSecondsList):  # Si el segundo no ha sido extraído
            usedSecond = 1
...
archivos = sorted(os.listdir(path), key=lambda f: float(re.search(r"(\d+\.\d+)", f).group()))
