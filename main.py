...
def main():
    ...
    command = ["python", "VoiceElseDuration.py", input_path, movie_name]
    try:
        subprocess.run(command, capture_output=True, text=True, check=True)
    except subprocess.CalledProcessError as e:
        print("An error occurred: ", e)
    ...
    ####### generate new subtitles for the summarized version - first approach, not properly working ######
    """
    os.chdir(main_path)
    command = ["python", "Newsubs.py", input_path, str(voice_speed), str(else_speed)]
    try:
        subprocess.run(command, capture_output=True, text=True, check=True)
    except subprocess.CalledProcessError as e:
        print("An error occurred: ", e)
    print("\n------- Step 7 - subtitles synced for the summarized movie --> COMPLETE ------\n")
    """
    ...
    os.chdir(main_path)
    command = ["python", "tozip.py", input_path, movie_name, reference]
    try:
        subprocess.run(command, capture_output=True, text=True, check=True)
    except subprocess.CalledProcessError as e:
        print("An error occurred: ", e)
    ...
    if restart == 'yes':
        ...
        command = ["python", "Restart.py", input_path, movie_name]
        try:
            subprocess.run(command, capture_output=True, text=True, check=True)
        except subprocess.CalledProcessError as e:
            print("An error occurred: ", e)
